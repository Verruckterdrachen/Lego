...
# ---------------------------------------------------------------------------
# Potts-регуляризация v3
# ---------------------------------------------------------------------------

# Сила сглаживания внутри однородных SLIC-зон (кожа, фон, волосы).
# Больше → ровнее тон, меньше «шума». Оптимум для портретов: 2.0–3.0.
# При > 4.0 начинают исчезать мелкие детали вне SLIC-границ.
POTTS_SMOOTHNESS = 2.5

# Число ICM-итераций. 3 достаточно для портретов 48×64–80×100.
# При > 4 прирост качества минимален, скорость падает.
POTTS_ITER = 3

# Сила локального FS-дизеринга в плавных зонах (0 = выключен).
# 0.35 — мягкое ступенчатое моделирование тона кожи без явной шахматки.
# 0.5 — хорошо заметная текстура, похожа на ручной арт.
# 0.6+ — дизеринг становится заметен, может выглядеть как шум.
POTTS_LOCAL_DITHER_STRENGTH = 0.40
