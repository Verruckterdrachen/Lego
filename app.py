...
if uploaded_file:
    ...
    with col_q2:
        ...
        bayer_step_multiplier = 1.8
        if quantize_mode in ("bayer4", "bayer8"):
            bayer_step_multiplier = st.slider(
                "Сила дизеринга Bayer (множитель шага)", min_value=0.3, max_value=4.0,
                value=1.8, step=0.1,
                help="Сдвиг калибруется от реального зазора между цветами палитры. "
                     "Ниже ~0.5 сетка Bayer практически не видна, выше 2.5 переходы становятся резче.",
            )
    ...
    colors_db_signature = (len(colors_df), tuple(colors_df["color_id"].tolist()),
                           ...
                           extra_smooth_enabled, extra_smooth_radius, bayer_step_multiplier)
